...
const rateLimit = (
	...
	const middleware = handleAsyncErrors(
		async (request: Request, response: Response, next: NextFunction) => {
			...
			// If we are to skip successful requests, delay the increment operation
			if (config.skipFailedRequests || config.skipSuccessfulRequests) {
				...
				if (await config.store.get!(key)) {
					...
					limit =
						typeof config.limit === 'function'
							? await config.limit(request, response)
							: config.limit
				...
				response.on('finish', async () => {
					if ((config.skipFailedRequests && await config.requestWasSuccessful(request, response)) || 
						(config.skipSuccessfulRequests && !(await config.requestWasSuccessful(request, response)))
						)
						await incrementAndHandleRateLimit(key, augmentedRequest, request, response, next, info, limit)
				})
			} else
				await incrementAndHandleRateLimit(key, augmentedRequest, request, response, next, undefined, undefined)

			next()
		},
	)

	async function incrementAndHandleRateLimit(key: string, augmentedRequest: AugmentedRequest, request: Request, response: Response, next: NextFunction, info?: RateLimitInfo, limit?: number) {
